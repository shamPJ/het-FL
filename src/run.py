...
from utils import mse_mean_std, mse_mean_std_scaled
...
if adj_matrix: n_neighbours, n_neighbours_fixed = args.n_neighbours, args.n_neighbours_fixed
...
# parent directory
path = '/scratch/work/abduras1/het-FL/out/Linreg_Torch_' + os.environ["SLURM_JOB_ID"]
# subdirs for each reg. term value
exp_dir =  path + '/reg_term_' + str(reg_term)
os.mkdir(exp_dir)
...
# out is the len(n_samples) list of tuples (mse_train_array, mse_val_array, mse_val_pooled_array, est_error_array, est_error_pooled_array)
mse_train_list, mse_val_list = [tpl[0] for tpl in out], [tpl[1] for tpl in out]
mse_val_pooled_list = [tpl[2] for tpl in out]
est_error_list, est_error_pooled_list = [tpl[3] for tpl in out], [tpl[4] for tpl in out]
...
np.save(exp_dir + '/stats/mse_std_scaled.npy', (mse_v_scaled, mse_std_v_scaled))
